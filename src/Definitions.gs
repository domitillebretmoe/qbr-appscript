...
function definitionRows() {
  ...
  return [
    ...
    ['Quarter actuals', 'QoQ', 'Metric this quarter - same metric the previous quarter. While the quarter is in progress the previous quarter is cut at the same elapsed day (only deals closed by then count, e.g. day 41 of Q3 vs day 41 of Q2; the section subtitle shows the cut-off date) so the comparison is like for like; once the quarter is closed it is full quarter vs full quarter. Metrics that depend on today's open pipeline or account state (open pipeline, coverage, expected logo attainment, conversion rate) have no same-point value and show "-"; Starting / Ending ARR are the ledger balances.', '-'],
