...
function definitionRows() {
  ...
  return [
    ...
    ['Quarter actuals', 'Net Added ARR', 'Sum of Delta ARR of Closed Won opportunities + Sum of Delta ARR of Closed Lost renewals. Delta ARR of a Closed Won non-renewal "MSP" deal whose NACV is an explicit 0 = its ARR (Salesforce ARR field, else Amount), since MSP deals are booked without NACV; a blank NACV stays 0 and is flagged under data quality.', 'Opportunity.NACV__c, ARR__c, Amount, Type, StageName'],
    ...
    ['Quarter actuals', 'QoQ', 'Metric this quarter - same metric the previous quarter. While the quarter is in progress the previous quarter is cut at the same elapsed day (only deals closed by then count, e.g. day 41 of Q3 vs day 41 of Q2; the section subtitle shows the cut-off date) so the comparison is like for like; once the quarter is closed it is full quarter vs full quarter. Metrics that depend on the current open pipeline or account state (open pipeline, coverage, expected logo attainment, conversion rate) have no same-point value and show "-"; Starting / Ending ARR are the ledger balances.', '-'],
    ...
    ['Quarter actuals', 'Pipeline by stage (tables, Q / Q+1 / Q+2)', 'Open opportunities with a close date in the quarter split by Salesforce stage: # opps, Sum of Delta ARR and each stage\'s share of the quarter\'s open pipeline (blank when the pipeline total is 0 or negative). Sorted by stage. Answers "how much of this quarter\'s pipe is still in early stages".', 'Opportunity.StageName, NACV__c, CloseDate'],
    ...
    ['Accounts (as of refresh)', `# Active Pilots / Active Pilots (table)`, `Open opportunities of the team in stage "${PILOT_STAGE}" at the time of the refresh, whatever their close date (a pilot may close in a later quarter). Table shows Salesforce Pilot Status and Pilot Expected Close Date. As-of metric: no QoQ or trend.`, 'Opportunity.StageName, IsClosed, Pilot_Status__c, Pilot_Expected_Close_Date__c'],
    ['Accounts (as of refresh)', '# Pilots Completed / Pilots Completed (table)', `Opportunities of the team with Pilot Status = ${PILOT_COMPLETE_STATUS} and a Pilot Actual End Date in the selected quarter (regardless of the opportunity\'s close date or current stage; the table shows the stage it is in now). QoQ same-point counts pilots ended by the comparison cut-off.`, 'Opportunity.Pilot_Status__c, Pilot_Actual_End_Date__c'],
    ...
    ['Rep activity & performance', 'Reps (table)', 'Active users with a User Segment in the US Majors / Europe / Asia / LATAM / Federal / US Enterprise / Partnerships groups whose Team is the tab\'s team (all member teams for a roll-up); Deployed Engineering, SDR and Pre-sales families excluded. Same measures as Salesforce > Global GTM Dashboard > Majors Rep Performance, scoped to the selected quarter (B2) up to today - the whole quarter once it is closed; goals stay per fiscal year (Feb-Jan) because Salesforce rep goals carry no quarter. One row per rep, best Won ARR (Q) first. Refreshed with the tab.', 'User_Segment__c (User__c, Team__c, Group__c), User.User_Family__c'],
    ...
    ['Rep activity & performance', 'Rep goal (FY)', 'Sum of the Weighted Goal of the Account Goals assigned to the rep (Account Goal owner, else the account\'s owner). Fiscal-year goal: Salesforce holds no quarterly rep goal, so no quarter goal is shown.', 'Account_Goal__c.Weighted_Goal__c, Account_Owner__c'],
    ['Rep activity & performance', 'Won ARR (Q / QTD)', 'Sum of Delta ARR of Closed Won opportunities owned by the rep closing in the selected quarter (same Delta ARR rule as Net Added ARR, incl. the MSP fallback to ARR).', 'Opportunity.OwnerId, IsWon, NACV__c, ARR__c, Amount, CloseDate'],
    ['Rep activity & performance', 'Won ARR (FY) / Attainment (FY)', 'Same, over the fiscal year of the selected quarter (Feb-Jan). Attainment = Won ARR (FY) / Rep goal (FY).', 'Opportunity.OwnerId, IsWon, NACV__c, ARR__c, Amount, CloseDate'],
    ['Rep activity & performance', 'Meetings (Q / QTD)', 'Distinct Gong-synced meetings (Events) owned by the rep dated within the selected quarter up to today, cancelled meetings excluded; an inactive duplicate user with the same name and email is credited to the rep.', 'Event.Gong__Gong_Activity_Id__c, Gong__Meeting_Prospect_Canceled__c, ActivityDate'],
    ['Rep activity & performance', 'Activities (Q / QTD)', 'Distinct Gong-synced Tasks (calls, emails) owned by the rep dated within the selected quarter up to today.', 'Task.Gong__Gong_Activity_Id__c, ActivityDate'],
    ['Rep activity & performance', 'Acct coverage (Q / QTD)', 'Share of the rep\'s owned accounts with at least one Gong-synced meeting or activity by the rep within the selected quarter up to today.', 'Event / Task.AccountId, Account.OwnerId'],
    ['Rep activity & performance', 'Pipeline created (Q / QTD)', 'Sum of Expected Delta ARR of opportunities owned by the rep created during the selected quarter (any stage).', 'Opportunity.CreatedDate, Expected_NACV__c'],
    ...
    ['Rep activity & performance', 'Renewal risk (FY)', 'ARR at risk / ARR up for renewal over the rep\'s open renewals in the fiscal year (Starting ARR > 0, not excluded from churn impact); ARR at risk = Starting ARR x (1 - Pwin).', 'Opportunity.Starting_ARR__c, Pwin__c, Exclude_From_Churn_Impact__c'],
    ...
    ['Owners & data quality', 'Owner performance (table)', 'Per opportunity owner for the selected quarter: Net Added ARR, # Closed Won, Churn ARR (downgrades + full churn), open pipeline in the quarter and open pipeline in Q+1. Sorted by Net Added ARR. Owners whose only opportunities are Closed Lost pipeline are omitted; an owner from another team (e.g. a UKI rep owning a deal on a DACH account, attributed to DACH by account team) is shown as "Name (team)".', 'Opportunity.OwnerId, User_Segment__c.Team__c'],
    ...
    ['Owners & data quality', 'Closed Won with $0 Delta ARR', 'Closed Won Land / Expand opportunity (not a renewal, not One Time, not an MSP deal with NACV 0 and an ARR) with Delta ARR = 0 or blank: probably missing NACV.', 'Opportunity.NACV__c, Type, RecordType'],
    ...
    ['Raw Data', 'Rows', 'Every Salesforce opportunity fetched for a team tab (all fiscal quarters from Q1-2026 to Q+2), one row each, with the Bucket it is counted in: Closed Won - Land/Expand, Won renewal, Won renewal - downgrade, Full churn, Lost pipeline, Open pipeline. Filter on Tab + Quarter to tie a metric out (pilots: Tab + Stage for active, Tab + Pilot Status + Pilot End Quarter for completed). Refreshing a tab replaces that tab\'s rows.', 'Raw Data tab'],
  ];
}
